export const CHANGE_TOP_BANNERS = "recommend/CHANGE_TOP_BANNERS";
export const HOT_RECOMMEND = "recommend/CHANGE_HOT_RECOMMEND";
export const NEW_RECOMMEND = "recommend/NEW_RECOMMEND";
export const PERSONAL_RECOMMEND = "recommend/PERSONAL_RECOMMEND";
export const CHANGE_NEW_LIST = "recommend/CHANGE_NEW_LIST";
export const CHANGE_UP_LIST = "recommend/CHANGE_UP_LIST";
export const CHANGE_ORIGIN_LIST = "recommend/CHANGE_ORIGIN_LIST";

export const CHANGE_SETTLE_SONGER = "recommend/CHANGE_SETTLE_SONGER";
